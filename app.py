...
st.markdown("""
    <div style='text-align:center; padding: 1.5rem 0 0.5rem;'>
        ...
        <h1 style='margin:0.3rem 0 0; font-size:2rem;'>Customer Churn Prediction</h1>
...
if st.button('🔮 Predict Churn'):
    ...
    with col_b:
        st.metric("Retention Probability", f"{1 - proba:.2%}")

    if proba > 0.5:
        st.error(f"⚠️ **High Risk!** This customer is likely to churn ({proba:.2%} probability).")
    else:
        st.success(f"✅ **Low Risk.** This customer is likely to stay ({(1-proba):.2%} retention probability).")
